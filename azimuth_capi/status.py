...
def _handle_node_addon_health(cluster):
...
def _timeout_if_stuck(cluster):
    if cluster.status.phase in {
        ...
        timeout = cluster.status.last_updated + dt.timedelta(
...
def _reconcile_cluster_phase(cluster):
    ...
    if _is_lease_blocking(cluster):
        ...
        if phase is None:
            phase = _handle_node_addon_health(cluster)
...
def control_plane_updated(cluster, obj):
    ...
    # want. The accurate version is in the status, but we use the spec if that is not
    # set yet.
