...
async def validate_cluster_template(name, spec, operation, **kwargs):
    ...
    if operation in {"CREATE", "UPDATE"}:
        ...
        clusters = ekresource.list(
            labels={f"{settings.api_group}/cluster-template": name}, all_namespaces=True
        )
        try:
            ...
            raise kopf.AdmissionError(
                "template is in use by at least one cluster", code=400
            )
...
async def validate_cluster(name, namespace, meta, spec, operation, **kwargs):  # noqa: C901
    ...
    if (
        current_cluster
        and current_cluster.spec.template_name == next_template.metadata.name
    ):
        return
...
async def adopt_lease(instance):
    ...
    lease = await ekleases.fetch(
        instance.spec.lease_name, namespace=instance.metadata.namespace
    )
    lease_patch = []
    ...
    if settings.api_group in existing_finalizers:
        await ekleases.patch(
            lease.metadata.name,
            {
                "metadata": {
                    "finalizers": [
                        f for f in existing_finalizers if f != settings.api_group
                    ],
                },
            },
    ...
    if settings.capi_helm.flavor_specific_node_group_overrides:
        ...
        for i in range(len(values["nodeGroups"])):
            ...
            for (
                pattern,
                overrides,
            ) in settings.capi_helm.flavor_specific_node_group_overrides.items():
...
async def adopt_oidc_client(instance: api.Cluster):
    ...
    """
    ekoidcclients = await ekclient.api(settings.identity.api_version).resource(
        "oidcclients"
    )
    try:
...
async def ensure_platform(instance: api.Cluster, realm):
    """
    Ensures that the platform resource for the cluster exists and matches the current
    services.
    """
    platform = {
        "apiVersion": settings.identity.api_version,
...
async def on_cluster_create(logger, instance, name, namespace, patch, **kwargs):  # noqa: C901
    """
    ...
    template = await fetch_model_instance(
        api.ClusterTemplate, instance.spec.template_name
    )
    if not template:
    ...
    if settings.identity.oidc_enabled:
        ...
        # Create the OIDC client for the cluster and wait for the issuer URL and client
        # ID to be available
    ...
    helm_values = generate_helm_values_for_release(
        template,
        instance,
        oidc_issuer_url,
        oidc_client_id,
    ...
    if instance.spec.lease_name:
        ...
        if lease_phase == "Active":
            ...
            helm_values["controlPlane"] = update_machine_flavor(
                helm_values["controlPlane"], size_map
            )
...
async def on_cluster_resume(instance, name, namespace, **kwargs):
    ...
    status.remove_unknown_nodes(
        ...
        [
            machine
            async for machine in ekmachines.list(labels=labels, namespace=namespace)
        ],
    )
    ...
    status.remove_unknown_addons(
        ...
        [
            ...
            async for addon in ekhelmreleases.list(
                namespace=namespace, labels={"capi.stackhpc.com/cluster": name}
            )
        ]
        + [
            addon
    ...
    **kwargs,
    ...
    def decorator(func):
        ...
        async def wrapper(**inner):
            ...
            # conflict kopf retry logic does not apply to events
            while True:
                try:
                    cluster = await fetch_model_instance(
                        api.Cluster, cluster_name, namespace=inner["namespace"]
                    )
                    if cluster:
...
@on_related_object_event(
    ...
    cluster_name_mapper=lambda obj: next(
        (
            ...
            if (
                ref["apiVersion"].split("/")[0] == settings.api_group
                and ref["kind"] == api.Cluster._meta.kind
            )
        ),
        None,
    ),
...
async def on_capi_machine_event(cluster, type, body, **kwargs):  # noqa: A002
    ...
    if type == "DELETED":
        ...
        infra_resource = await ekclient.api(infra_ref["apiVersion"]).resource(
            infra_ref["kind"]
        )
        infra_machine = await infra_resource.fetch(
            infra_ref["name"], namespace=infra_ref["namespace"]
        )
    ...
    while True:
        try:
            ...
            platform = {
                ...
                "kind": "Platform",
                "metadata": {
                    "name": settings.identity.app_platform_name_template.format(
                        app_name=name
                    ),
                    "namespace": namespace,
                },
            ...
            logger.error(f"{str(exc)} - retrying")  # noqa: RUF010
        except Exception:
...
async def annotate_addon_for_reservation(
    cluster_name, cluster_namespace, reservation, service_name, service_status=None
):
    """
    ...
    # Note that this function will not run concurrently for two reservations on the
    # same cluster, so this is a safe operation
...
async def monitor_cluster_services(instance, name, namespace, **kwargs):
    ...
    eksecrets = await ekclient.api("v1").resource("secrets")
    try:
        kubeconfig = await eksecrets.fetch(f"{name}-kubeconfig", namespace=namespace)
    except ApiError as exc:
        if exc.status_code == 404:
...
async def reconcile_app_template(instance, param, **kwargs):
    ...
    next_versions = []
    # For each version, we need to make sure we have a values schema and optionally a
    # UI schema
